...
std::vector<write_task> task_builder::build_write_task(write_task task,
    ...
    auto number_of_tasks = static_cast<int>(std::ceil((float)
            (source.offset + source.size) / MAX_IO_UNIT));
    auto dataspace_id = static_cast<int64_t>
            (std::chrono::duration_cast<std::chrono::microseconds>
            (std::chrono::system_clock::now().time_since_epoch()).count());
    std::size_t chunk_index = source.offset / MAX_IO_UNIT;
    std::size_t base_offset = chunk_index * MAX_IO_UNIT +
...
/*************************** chunk in dataspace ******************************/
            if(cm.destination.location == location_type::CACHE){
                ...
                chunk_value.replace(base_offset,
                                    MAX_IO_UNIT - base_offset,
                ...
                map_client->put(
                        ...
                        chunk_value);
                /********* build new task **********/
                sub_task->destination.size=cm.destination.size;
