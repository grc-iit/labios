...
server) {
    ...
    t.resumeTime();
#endif
    auto return_value= labios_system::getInstance(service_i)->map_client()->
            get(name,key,server);
#ifdef TIMERDM
...
server) {
    return  labios_system::getInstance(service_i)->map_client()->exists(name,key,server);
}
...
server) {
    ...
    t.resumeTime();
#endif
    auto return_value = labios_system::getInstance(service_i)
