...
static void pfree(void *p) { /* semi-own free routine which makes additional test */
    if (p == NULL) printf("freeing null");
    free(p);
}


static void *palloc(long size, long no) { /* semi-own alloc routine which makes additional test */
    long *p;
    size *= no;
    if (size == 0) size = 1;
...
static void pop(allinfo *a, int side, item **f, item **l) {
    ...
    switch (side) {
        case LEFT :
            if (a->intv1 == a->intv1b) printf("pop left");
            (a->intv1)--;
            pos = a->intv1;
            break;
        case RIGHT:
            if (a->intv2 == a->intv2b) printf("pop right");
            (a->intv2)++;
            pos = a->intv2;
            break;
    }
...
static void wadd(allinfo *a, item *t) {
    ...
    /* initialize limits */
    if (2 * a->db.size + 1 > SSSTATES) printf("no space in wadd");
...
static void wsub(allinfo *a, item *s) {
    ...
    /* initialize limits */
    if (2 * a->da.size + 1 > SSSTATES) printf("no space in wsub");
...
static void mulbranch(allinfo *a, stype *c1) {
    ...
    if ((ub > a->z1) && (a->h < a->litem) && (csum > 0)) {
        ...
        if ((a->h < a->litem) && (j != NULL)) {
            ...
            if (j == a->litem + 1) printf("cannot find %hd,%hd", p, w);
            j->a = a1;
        }
        a->curred -= (a->h - h);
...
static int mulknap(int n, int m, int *p, int64_t *w, int *x, int64_t *c) {
    ...
    a.csum = 0;
    for (i = 1; i != m; i++) { if (c[i - 1] > c[i]) printf("c not ordered"); }
    for (i = 0; i != m; i++) {
        a.cap[i] = c[i];
        a.csum += c[i];
    }
