"""
This module provides classes and methods to launch the MontageKern application.
MontageKern is ....
"""
...
class MontageKern(Application):
    """
    This class provides methods to launch the MontageKern application.
    """
    def _init(self):
        """
        Initialize paths
    ...
    def _configure_menu(self):
        ...
        return [
            ...
            },
            {
                ...
                'type': str,
                'default': '/tmp/montage_kern/',
            },
            {
                'name': 'final_path',
                'msg': 'The path to the final output file directory',
                'type': str,
                'default': '/tmp/montage_kern_final/',
            },
            {
                'name': 'iterations',
    ...
    def _configure(self, **kwargs):
        ...
        if self.config['final_path'][-1] != '/':
            self.config['final_path'] += '/'
        Mkdir(self.config['file_path'], PsshExecInfo(hostfile=self.jarvis.hostfile))
        Mkdir(self.config['final_path'], PsshExecInfo(hostfile=self.jarvis.hostfile))
    ...
    def start(self):
        ...
        if self.config['mode'] == 'base':
            exec_path = 'montage_base'
        else:
            exec_path = 'montage_tabios'
    ...
    def clean(self):
        ...
        Rm(f"{self.config['final_path']}/*", PsshExecInfo(hostfile=self.jarvis.hostfile))
        Rm(self.config['final_path'], PsshExecInfo(hostfile=self.jarvis.hostfile)) 